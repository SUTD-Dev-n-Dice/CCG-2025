...
func _physics_process(delta):
	...
	if back_on:
		...
		if not pause_input and Input.is_action_pressed("p2_right"):
			direction.x += 1
			if scale.x == -1:
				dash_timer.start()
			if scale.x == 1:
		...
		if not pause_input and Input.is_action_pressed("p2_left"):
			direction.x -= 1
			if scale.x == -1:
				back_timer.start()
			if scale.x == 1:
	...
	if is_on_floor() and Input.is_action_just_pressed("p2_up"):
