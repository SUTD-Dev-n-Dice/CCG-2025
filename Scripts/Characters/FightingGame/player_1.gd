...
func _input(event):
	...
	if Input.is_action_just_released("right1"):
		dash_on = false

	if Input.is_action_just_released("left1"):
		dash_on = false
...
func _physics_process(delta):
	...
	if back_on:
		if airdash_on == false:
			...
			move_and_slide()
		elif airdash_on == true:
			target_velocity
	else:
		# We check for each move input and update the direction accordingly.
		if not pause_input and Input.is_action_pressed("right1"):
		...
		if not pause_input and Input.is_action_pressed("left1"):
			direction.x -= 1
			if scale.x == 1:
				back_timer.start()
			if scale.x == -1:
	...
	if is_on_floor() and Input.is_action_just_pressed("up1"):
